...
def detect_liveness(img):
    ...
    for (x, y, w, h) in faces:
        ...
        for (ex, ey, ew, eh) in eyes:
            ...
            if eye_aspect_ratio(eye) < 0.2:  # Adjust the threshold as needed
                return False

    return True

def preprocess_image(img):
    alpha = 1.2  # Simple brightness factor
...
def train_model():
    ...
    for user in userlist:
        for imgname in os.listdir(f'static/faces/{user}'):
            ...
            if detect_liveness(img):
                resized_face = cv2.resize(img, (50, 50))
                preprocessed_face = preprocess_image(resized_face)
                faces.append(preprocessed_face.flatten())
                labels.append(user)

    if not faces or not labels:
        print("No live faces found. Please check if the data is properly loaded.")
        return
